...
vector<Point2> getRootEllipse(float cx,float cy,float r,float a, float b, float e,vector<float>& yr);
...
double triangleArea(Point2 start, Point2 end, Point2 orig)
{
  Point2 temp1 = start - orig;
  Point2 temp2 = end - orig;
  double area = abs(temp1.m_X*temp2.m_Y-temp2.m_X*temp1.m_Y)/2;
...
double getEllipseSegArea(Point2 p1,Point2 p2, Point2 orig,double a, double b)
{
  ...
  double areaE = ellipseSectionArea(p1,p2,orig, a, b) + signE*triangleArea(p1,p2,orig);
  return areaE;
}
...
float findInterEllipseCycle(Point h1,Point h2,Point c1,int ah,int h3,float avgHopDis1,float avgHopDis2,vector<float>& yr )
{
  ...
  vector<Point2> result = getRootEllipse(c2x, c2y, r, a, b, c,yr);
  ...
  /*
	...
	*/
	...
	double ry =0;
	double area =0;
  Point2 newOrig = {cx, cy};
  Point2 orig = {0.0,0.0};
	if(vsize==0 || vsize == 1)
	{
	  //case 3,6 seperate
	  if(euDis2(ex,ey,cx,cy)>=r && euDis2(lx,ly, cx,cy)+euDis2(rx,ry,cx,cy)>=2*a){
	    area = 0;
	  ...
	  if(vsize ==2 ){
	    ...
	    area = min(getEllipseSegArea(result[0],result[1],orig,a,b),getCircleSegArea(result[0],result[1],newOrig, r));
	    area += min(getEllipseSegArea(result[1],result[0],orig,a,b),getCircleSegArea(result[1],result[0],newOrig, r));
	    //cout<<"area: "<<area<<endl;
...
vector<Point2> getRootEllipse(float cx,float cy,float r,float a, float b, float e,vector<float>& yr)
{
    ...
    vector<Point2> result;
		if(abs(cx)<0.0001 && abs(cy)<0.0001){
       ...
       */
		    ...
		    for( int i =0;i<4;i++){
		      ...
		      if(abs(solutions[i].imag())<0.0001 && abs(preY-res)>0.0001 && (b*b-res*res)>=0 && (r*r-(res-cy)*(res-cy))>=0){
		        temp.push_back(res);
		        preY = res;
		      }		    
		    }
		    ...
		    for(vector<double>::iterator it = temp.begin(); it != temp.end(); ++it){
		      double y = *it;
		      double res1 = sqrt(a*a*(1-y*y/b/b));
		      double res2 = -sqrt(a*a*(1-y*y/b/b));
		      if( abs((res1-cx)*(res1-cx)+(y-cy)*(y-cy)-r*r)<0.1 ){
		        Point2 tempPoint = {res1, y};
		        result.push_back(tempPoint);
		      }
		      if(abs((res2-cx)*(res2-cx)+(y-cy)*(y-cy)-r*r)<0.1 && abs(res1-res2)>0.1){
